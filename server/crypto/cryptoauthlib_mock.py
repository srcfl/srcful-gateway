# mocking all the crypto functions
...
def atcab_info_base(mode, param2, out_data):
    ...
    memmove(cast(out_data, c_void_p).value, cast(byref(r_revision), c_void_p).value, len(r_revision))
...
def atcab_read_serial_number(serial_number):
  ...
  r_ser_num.value = bytes(bytearray([0xd0, 0x0e, 0x0a, 0x0d, 0x0b, 0x0e, 0x0e, 0x0f, 0x00]))
  ...
  if not isinstance(serial_number, bytearray):
      ...
      memmove(cast(c_serial_number, c_void_p).value, cast(byref(r_ser_num), c_void_p).value, len(r_ser_num))
