...
def test_handle_import_when_max_charging():
    ...
    grid_power = total_household_load + instantaneous_battery_power + pv_power
    power, state = handle_import(grid_power, grid_import_power_limit, instantaneous_battery_power, battery_soc, battery_max_charge_discharge_power, min_battery_soc)
...
def test_handle_import_exact_power_match():
    ...
    power, state = handle_import(grid_power, grid_import_power_limit, instantaneous_battery_power, battery_soc, battery_max_charge_discharge_power, min_battery_soc)
    print(f"Power: {power}, State: {state}")
    assert power == 0  # Should discharge at max power
    assert state == State.NO_ACTION
...
def test_handle_import_moderate_charging_state():
    ...
    grid_power = total_household_load + instantaneous_battery_power + pv_power  # 15000W
    power, state = handle_import(grid_power, grid_import_power_limit, instantaneous_battery_power, battery_soc, battery_max_charge_discharge_power, min_battery_soc)
    print(f"Power: {power}, State: {state}")
...
def test_handle_import_with_pv_generation():
    ...
    grid_power = total_household_load + instantaneous_battery_power + pv_power  # 13000W
    power, state = handle_import(grid_power, grid_import_power_limit, instantaneous_battery_power, battery_soc, battery_max_charge_discharge_power, min_battery_soc)
    print(f"Power: {power}, State: {state}")
