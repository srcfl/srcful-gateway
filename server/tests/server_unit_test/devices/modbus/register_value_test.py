...
def test_decode_value_single_register_with_scale_factor():
    """Test RegisterInterval.decode_value() with single register and scale factor from harvest data"""
    from server.devices.supported_devices.profile import RegisterInterval
    from server.devices.profile_keys import FunctionCodeKey, DataTypeKey, EndiannessKey

    # Harvest data for register 0
    harvest = {"0": 4999}

    reg_0 = RegisterInterval(
        function_code=FunctionCodeKey.READ_INPUT_REGISTERS,
        start_register=0,
        offset=1,
        data_type=DataTypeKey.U16,
        unit="Hz",
    ...
    # Set raw registers from harvest data and decode
    reg_0.raw_registers = [harvest[str(0 + i)] for i in range(reg_0.offset)]
    assert reg_0.raw_registers == [4999]
    raw_bytes, decoded_value = reg_0.decode_value()

    assert reg_0.decoded_value == 49.99  # 4999 * 0.01
    assert decoded_value == 49.99
    assert raw_bytes.hex() == "1387"  # 4999 in hex


def test_decode_value_single_register_no_scale():
    """Test RegisterInterval.decode_value() with single register and no scale factor from harvest data"""
    from server.devices.supported_devices.profile import RegisterInterval
    from server.devices.profile_keys import FunctionCodeKey, DataTypeKey, EndiannessKey

    # Harvest data for register 609
    harvest = {"609": 4995}

    reg_609 = RegisterInterval(
        function_code=FunctionCodeKey.READ_INPUT_REGISTERS,
        start_register=609,
        offset=1,
        data_type=DataTypeKey.U16,
    ...
    # Set raw registers from harvest data and decode
    reg_609.raw_registers = [harvest[str(609 + i)] for i in range(reg_609.offset)]
    raw_bytes, decoded_value = reg_609.decode_value()

    assert reg_609.decoded_value == 4995.0
    assert decoded_value == 4995.0
    assert raw_bytes.hex() == "1383"  # 4995 in hex


def test_decode_value_multi_register_u32():
    """Test RegisterInterval.decode_value() with multi-register U32 from harvest data"""
    from server.devices.supported_devices.profile import RegisterInterval
    from server.devices.profile_keys import FunctionCodeKey, DataTypeKey, EndiannessKey

    # Harvest data for registers 672-673
    harvest = {"672": 0, "673": 158}

    reg_672 = RegisterInterval(
        function_code=FunctionCodeKey.READ_INPUT_REGISTERS,
        start_register=672,
        offset=2,
        data_type=DataTypeKey.U32,
    ...
    # Set raw registers from harvest data and decode
    reg_672.raw_registers = [harvest[str(672 + i)] for i in range(reg_672.offset)]
    raw_bytes, decoded_value = reg_672.decode_value()

    assert reg_672.decoded_value == 158.0  # 0 << 16 | 158 = 158
    assert decoded_value == 158.0
    assert raw_bytes.hex() == "0000009e"  # 0x0000009e = 158


def test_decode_value_zero_with_scale():
    """Test RegisterInterval.decode_value() with zero value and scale factor from harvest data"""
    from server.devices.supported_devices.profile import RegisterInterval
    from server.devices.profile_keys import FunctionCodeKey, DataTypeKey, EndiannessKey

    # Harvest data for register 674
    harvest = {"674": 0}

    reg_674 = RegisterInterval(
        function_code=FunctionCodeKey.READ_INPUT_REGISTERS,
        start_register=674,
        offset=1,
        data_type=DataTypeKey.U16,
    ...
    # Set raw registers from harvest data and decode
    reg_674.raw_registers = [harvest[str(674 + i)] for i in range(reg_674.offset)]
    raw_bytes, decoded_value = reg_674.decode_value()

    assert reg_674.decoded_value == 0.0  # 0 * 0.1
    assert decoded_value == 0.0
    assert raw_bytes.hex() == "0000"
