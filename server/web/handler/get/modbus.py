...
class ModbusHandler(GetHandler):
    ...
    def do_get(self, request_data: RequestData) -> Tuple[int, str]:
        try:
            ...
            try:
                # Convert to int, whether decimal or hex
                if isinstance(raw_function_code, str) and raw_function_code.startswith('0x'):
                    function_code_int = int(raw_function_code, 16)
                else:
                    function_code_int = int(raw_function_code)
            except ValueError:
                return 400, json.dumps({"error": f"function code must be a number"})

            # Map common decimal values to their hex equivalents
            function_code_map = {
            ...
            # Convert decimal to hex equivalent if needed
            function_code_int = function_code_map.get(function_code_int, function_code_int)
    ...
    def _handle_write(self, request_data: RequestData, device, address: int, function_code: FunctionCodeKey) -> Tuple[int, str]:
        ...
        logger.debug(f"Write operation - address: {address}, device_id: {device.get_SN()}, function_code: {function_code}, values: {values}")
        ...
        logger.debug(f"Write operation successful: {ret}")
