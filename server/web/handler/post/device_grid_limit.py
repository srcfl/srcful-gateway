...
class Handler(PostHandler):
    def schema(self):
        return {
            "type": "post",
            "description": "Set grid power limit in Watts for a specific device",
    ...
    def do_post(self, data: RequestData):
        ...
        try:
            ...
            if limit <= 0:
                return 400, json.dumps({"status": "error", "message": "Grid power limit must be positive"})
        except ValueError:
        ...
        for device in data.bb.devices.lst:
            if device.sn == device_sn:
                ...
                if dee_decoder:
                    dee_decoder.set_grid_power_limit(limit)
                    logger.info(f"Set grid power limit to {limit}W for device {device_sn}")
                    return 200, json.dumps({"status": "success", "message": f"Grid power limit set to {limit}W for device {device_sn}"})
                else:
