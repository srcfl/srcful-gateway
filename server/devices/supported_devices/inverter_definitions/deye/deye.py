...
class DeyeProfile(ModbusProfile):
    ...
    def _get_battery(self, device_sn: str, timestamp_ms: int, decoded_registers: List[RegisterInterval], hv_lv_power_sf: float, hv_lv_voltage_sf: float) -> EBatteryType:
    ...
    def _get_esystem_data(self, device_sn: str, timestamp_ms: int, harvest: dict) -> List[EBaseType]:
        ...
        esystem_data.append(self._get_metadata(device_sn, timestamp_ms, decoded_registers))
        esystem_data.append(self._get_grid(device_sn, timestamp_ms, decoded_registers))
        esystem_data.append(self._get_battery(device_sn, timestamp_ms, decoded_registers, hv_lv_power_sf, hv_lv_voltage_sf))
        esystem_data.append(self._get_solar(device_sn, timestamp_ms, decoded_registers, hv_lv_power_sf))
        esystem_data.append(self._get_load(device_sn, timestamp_ms, decoded_registers))
