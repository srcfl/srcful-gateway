...
class Device(ICom, ABC):
    ...
    def get_backoff_time_ms(self, harvest_time_ms: int, previous_backoff_time_ms: int) -> int:
        ''' computes a backoff time based, this is always between 5 and 256 seconds,
        it will increase if the harvest time is high and incrementatlly move towards 5000ms if the harvest time is low'''
        min_backoff_time = max(harvest_time_ms * 2, 5000)
